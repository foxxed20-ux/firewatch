...
def _root(model_dir=None):
    return Path(
        ...
        or Path(__file__).resolve().parents[1] / "model_bundle"
    ).resolve()


def _verify_spec(root, spec):
    if spec["backend"] == "ensemble":
        if set(spec["components"]) != {"cnn", "tree"}:
            raise ValueError("Incomplete ensemble")
        if not 0 <= float(spec["tree_weight"]) <= 1:
            raise ValueError("Invalid ensemble weight")
        for component in spec["components"].values():
            _verify_spec(root, component)
        return
    if spec["backend"] not in ("lightgbm", "torch"):
        raise ValueError("Unknown model backend")
    folder = (root / spec["path"]).resolve()
    if not folder.is_relative_to(root):
        raise ValueError("Unsafe model path")
    required = (
        {"model.txt", "metadata.json"} if spec["backend"] == "lightgbm" else {"best.pt"}
    )
    if not required.issubset(spec["sha256"]):
...
def describe_models(model_dir=None):
    ...
    if not path.is_file():
        return {
            ...
            "bundle_id": None,
            "tasks": {},
            "reason": "Model bundle has not been installed",
        }
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        if set(data["tasks"]) != {"af", "bs"}:
            raise ValueError("Both AF and BS are required")
        if data["feature_version"] != FEATURE_VERSION:
            raise ValueError("Incompatible features")
        for spec in data["tasks"].values():
            _verify_spec(root, spec)
...
def _load(root_string, task, manifest_digest):
    ...
    if spec["backend"] == "ensemble":
        parts = {
            k: _load_component(root, task, v) for k, v in spec["components"].items()
        }
        return (
            manifest,
            spec,
            {k: v[0] for k, v in parts.items()},
            {k: v[1] for k, v in parts.items()},
            {k: v[2] for k, v in parts.items()},
        )
...
def _load_component(root, task, spec):
    ...
    if spec["backend"] == "lightgbm":
        ...
        return (
            lgb.Booster(model_file=str(folder / "model.txt")),
    ...
    if spec["backend"] == "torch":
        ...
        checkpoint = torch.load(
            folder / "best.pt", map_location="cpu", weights_only=True
        )
...
def _probabilities(chip, spec, model, meta, device):
    expected = (
        meta["feature_names"]
        if spec["backend"] == "lightgbm"
        else meta["feature_meta"]["names"]
    )
    if chip.feature_names != expected:
        raise ValueError("Ordered feature names disagree with model")
    if spec["backend"] == "lightgbm":
        ...
        with torch.inference_mode():
            ...
            p = (
                (logits.sigmoid() if chip.task == "af" else logits.softmax(1))[0]
                .cpu()
                .numpy()
            )
...
def predict_features(chip, model_dir=None, bundle_id=None):
    ...
    manifest, spec, model, meta, device = _load(
        str(root), chip.task, hashlib.sha256(raw).hexdigest()
    )
    if bundle_id and bundle_id != manifest["bundle_id"]:
        raise ValueError(f"Unavailable bundle {bundle_id}")
    if spec["backend"] == "ensemble":
        scores = {
            k: _probabilities(chip, component, model[k], meta[k], device[k])
            for k, component in spec["components"].items()
        }
        weight = float(spec["tree_weight"])
        score = ((1 - weight) * scores["cnn"] + weight * scores["tree"]).astype(
            np.float32
        )
        ...
        postprocess = spec.get("postprocess", meta)
        hashes = spec["sha256"]
    if chip.task == "af":
        mask = (score >= float(postprocess["threshold"])).astype(np.uint8)
    else:
        multipliers = np.asarray(
            postprocess.get("class_multipliers", [1, 1, 1, 1]), np.float32
        )
        if (
            multipliers.shape != (4,)
            ...
            raise ValueError("Invalid class multipliers")
        mask = (score * multipliers[:, None, None]).argmax(0).astype(np.uint8)
    if (
        mask.shape != chip.x.shape[1:]
        or not np.isin(mask, [0, 1] if chip.task == "af" else [0, 1, 2, 3]).all()
    ):
        raise RuntimeError("Invalid model output")
    return {
        "class_map": mask,
        "observation_valid": chip.observation_valid,
        "score": score,
        "provenance": {
            ...
            "cloud_policy": "All pixels predicted; observation_valid is informational, never an output mask",
            "model_sha256": hashes,
        },
    }
...
def predict_chip(dataset_root, chip_id, bundle_id=None, model_dir=None):
    ...
    if chip_id not in inputs:
        raise FileNotFoundError(f"Chip {chip_id} not found")
