...
def _npz(path: Path, preferred: str) -> np.ndarray:
    with np.load(path, allow_pickle=False) as archive:
        return np.asarray(
            archive[preferred] if preferred in archive else archive["arr_0"]
        )
...
def load_records(path: str | Path, task: str | None = None) -> list[Record]:
    ...
    for i, row in enumerate(rows):
        ...
        if record_id in seen_ids:
            raise ValueError(
                f"duplicate record id {record_id!r} in {seen_ids[record_id]!r} and {split!r}"
            )
        ...
        valid_path = (
            _path(str(row["valid_path"]), manifest.parent).resolve()
        ...
        for labeled_path in (x_path, y_path, valid_path):
            ...
            if labeled_path in data_splits and data_splits[labeled_path] != split:
                raise ValueError(
                    f"data path {labeled_path} occurs in both train and val"
                )
...
def _read_record(record: Record) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not np.issubdtype(raw_y.dtype, np.integer):
        raise ValueError(
            f"{record.id}: labels must be integer dtype, got {raw_y.dtype}"
        )
    ...
    if x.ndim != 3 or y.ndim != 2 or x.shape[1:] != y.shape or valid.shape != y.shape:
        raise ValueError(
            f"{record.id}: expected x CHW and y/valid HW, got {x.shape}/{y.shape}/{valid.shape}"
        )
...
def _binary_loss(
    logits: torch.Tensor, target: torch.Tensor, valid: torch.Tensor, pos_weight: float
) -> torch.Tensor:
    mask = valid.float()
    ...
    dice = 1 - (2 * inter + 1) / (
        (probability * mask).sum() + (target_f * mask).sum() + 1
    )
    return bce + dice
...
def _class_balance(records: list[Record], task: str) -> tuple[float, torch.Tensor]:
    ...
    for r in records:
        ...
        for c in range(len(counts)):
            counts[c] += int(np.sum(valid & (y == c)))
    if task == "af":
        return float(np.clip(counts[0] / max(counts[1], 1), 1, 30)), torch.ones(1)
    weights = counts.sum() / np.maximum(counts, 1) / len(counts)
    return 1.0, torch.tensor(np.clip(weights, 0.25, 8), dtype=torch.float32)


def _validate(
    model: nn.Module, loader: DataLoader, task: str, device: torch.device, amp: bool
    ...
    with torch.no_grad():
        for x, y, valid in loader:
            ...
            raw = (
                ...
                if task == "af"
                else torch.argmax(logits, 1).cpu().numpy()
            )
            for truth, permitted, predicted in zip(y.numpy(), valid.numpy(), raw):
                ...
                if task == "af":
                    ...
                    predictions.append(
                        np.where(permitted, predicted, 0).astype(np.uint8)
                    )
                if task != "af":
                    refs.append(np.where(permitted, truth, 0).astype(np.uint8))
    if task == "af":
        best_threshold, best_f1 = _best_af_threshold(refs, probabilities)
        return {"F1_af": best_f1, "threshold": best_threshold}, best_f1
    iou_burn = (
        float(micro_iou_burn(refs, predictions))
        if micro_iou_burn
        else _burn_iou(refs, predictions)
    )
    sev = (
        micro_iou_severity(refs, predictions)
        if micro_iou_severity
        else {c: _class_iou(refs, predictions, c) for c in (1, 2, 3)}
    )
    ...
    torch.save(
        {
            ...
            "model": {
                ...
                "out_channels": 1 if task == "af" else 4,
                "base_channels": base,
                "task": task,
            },
            "model_state": model.state_dict(),
            "normalizer": normalizer,
            "feature_meta": {"names": list(feature_names)},
            "epoch": epoch,
...
def train_task(
    ...
    *,
    ...
    if any(r.task != task for r in records):
        raise ValueError("train_task takes one task")
    train_records = [r for r in records if r.split == "train"]
    val_records = [r for r in records if r.split == "val"]
    if not train_records or not val_records:
        raise ValueError(f"{task}: both train and val records are required")
    x0, _, _ = _read_record(train_records[0])
    channels = x0.shape[0]
    if any(_read_record(r)[0].shape[0] != channels for r in records):
        raise ValueError(f"{task}: channel count differs between records")
    feature_names = train_records[0].feature_names
    if (
        not feature_names
        or len(feature_names) != channels
        or len(set(feature_names)) != channels
    ):
        raise ValueError(
            f"{task}: every record requires unique ordered feature_names for all channels"
        )
    ...
    train_loader = DataLoader(
        ChipDataset(train_records, normalizer, True, crop_size),
    ...
    val_loader = DataLoader(
        ChipDataset(val_records, normalizer, False, crop_size),
        batch_size=1,
        shuffle=False,
        num_workers=workers,
        pin_memory=amp,
    )
    pos_weight, class_weights = _class_balance(train_records, task)
    ...
    for epoch in range(1, epochs + 1):
        if epoch > 1 and time.monotonic() - started >= time_limit_min * 60:
            break
        model.train()
        losses = []
        for x, y, valid in train_loader:
            ...
            with torch.autocast(
                ...
                loss = (
                    _binary_loss(model(x), y, valid, pos_weight)
                    if task == "af"
                    else _multiclass_loss(model(x), y, valid, class_weights)
                )
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="Train FireWatch AF/BS segmentation models from preprocessed NPZ chips"
    )
    ...
    p.add_argument("--time-limit-min", type=float, default=55)
    ...
    root = Path(a.output)
    tasks = ("af", "bs") if a.task == "all" else (a.task,)
    result = []
    for task in tasks:
        result.append(
            train_task(
