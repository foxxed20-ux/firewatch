...
def _read(path: Path, expected: tuple[str, ...], size: int, dtype: str):
    with rasterio.open(path) as ds:
        if ds.count != len(expected) or (ds.height, ds.width) != (size, size):
            raise ValueError(
                f"Unexpected shape for {path.name}: {(ds.count, ds.height, ds.width)}"
            )
        names = tuple(ds.descriptions)
        if names != expected:
            raise ValueError(
                f"Band descriptions for {path.name}: {names}, expected {expected}"
            )
        if any(np.dtype(item) != np.dtype(dtype) for item in ds.dtypes):
            raise ValueError(
                f"Unexpected dtype for {path.name}: {ds.dtypes}; expected {dtype}"
            )
...
def discover_chips(data_root: str | Path) -> dict[str, dict[str, Path]]:
    ...
    suffixes = {
        "VIIRS_I1-I5": "viirs",
        "AUX": "aux",
        "Sentinel-2_pre": "s2_pre",
        "Sentinel-2_post": "s2_post",
        "Sentinel-1_pre": "s1_pre",
        "Sentinel-1_post": "s1_post",
    }
    for p in root.rglob("*.tif"):
        for suffix, key in suffixes.items():
            ...
            if p.name.endswith(ending):
                ...
                if not re.fullmatch(r"(AF|BS)_[A-Za-z0-9_-]+", chip_id):
                    continue
                paths = result.setdefault(chip_id, {})
                if key in paths:
                    raise ValueError(
                        f"Duplicate input {chip_id}/{key}; pass a single train or test root"
                    )
                paths[key] = p
                break
...
def _ratio(a, b):
    return np.clip((a - b) / (np.abs(a) + np.abs(b) + 1e-5), -1, 1)
...
def read_chip(chip_id: str, paths: dict[str, Path]) -> Chip:
    ...
    def add(name, value):
        names.append(name)
        features.append(
            np.nan_to_num(value, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32)
        )

    def read(key, expected, size):
        if key not in paths:
            raise FileNotFoundError(f"Missing {chip_id}/{key}")
        dtype = (
            "float32"
            if task == "af"
            else ("uint16" if key.startswith("s2_") else "int16")
        )
        a, g = _read(paths[key], expected, size, dtype)
        grids.append(g)
        return a

    if task == "af":
        v = read("viirs", VIIRS, 256)
        aux = read("aux", AF_AUX, 256)
        if not np.isin(v[7][np.isfinite(v[7])], [0, 1]).all():
            raise ValueError("VIIRS valid band must be binary")
        valid = np.isfinite(v[:5]).all(axis=0) & (v[7] > 0.5)
        for i, name in enumerate(VIIRS):
            value = v[i]
            if name in ("I4", "I5"):
                value = (value - 273.15) / 50
            elif "zenith" in name:
                value = value / 90
            add(name, value)
        ...
        pre = read("s2_pre", S2, 512)
        post = read("s2_post", S2, 512)
        if (
            not np.isin(pre[9], range(12)).all()
            or not np.isin(post[9], range(12)).all()
        ):
            raise ValueError("Invalid SCL classes")
        sarpre = read("s1_pre", ("VV", "VH"), 512) / 100
        sarpost = read("s1_post", ("VV", "VH"), 512) / 100
        aux = read("aux", BS_AUX, 512)
        valid = (
            np.isfinite(pre).all(axis=0)
            & np.isfinite(post).all(axis=0)
            & ~np.isin(pre[9], [0, 1, 3, 8, 9, 10, 11])
            & ~np.isin(post[9], [0, 1, 3, 8, 9, 10, 11])
        )
        pre[:9] /= 10000
        post[:9] /= 10000
        for date, a in [("pre", pre), ("post", post)]:
            for i, name in enumerate(S2):
                add(f"{date}_{name}", a[i] / 11 if name == "SCL" else a[i])
        for date, a in [("pre", sarpre), ("post", sarpost)]:
            for i, name in enumerate(("VV", "VH")):
                add(f"{date}_{name}_db", a[i] / 30)
        add("dem", aux[0] / 1000)
        add("slope", aux[1] / 90)
        add("landcover", aux[2] / 100)
        for i, name in enumerate(S2[:9]):
            add(f"delta_{name}", pre[i] - post[i])
        for i, name in enumerate(("VV", "VH")):
            add(f"delta_{name}_db", (sarpre[i] - sarpost[i]) / 20)
        delta = {}
        for name, i, j in [
            ("NBR", 6, 8),
            ("NDVI", 6, 2),
            ...
            a = _ratio(pre[i], pre[j])
            b = _ratio(post[i], post[j])
            delta[name] = a - b
            add(f"pre_{name}", a)
            add(f"post_{name}", b)
            add(f"delta_{name}", a - b)
        for size in (3, 9):
            add(
                f"dNBR_mean_{size}",
                uniform_filter(delta["NBR"], size=size, mode="reflect"),
            )
            add(
                f"dNDVI_mean_{size}",
                uniform_filter(delta["NDVI"], size=size, mode="reflect"),
            )
        add(
            "sar_VH_delta_mean_5",
            uniform_filter((sarpre[1] - sarpost[1]) / 20, size=5, mode="reflect"),
        )
    else:
        raise ValueError(f"Unknown task: {chip_id}")
    if any(g != grids[0] for g in grids[1:]):
        raise ValueError(f"Input grids disagree for {chip_id}")
    return Chip(
        chip_id,
        task,
        np.stack(features),
...
def read_training_mask(chip_id: str, paths: dict[str, Path]) -> np.ndarray:
    ...
    with rasterio.open(sample) as ds:
        input_grid = (ds.width, ds.height, ds.crs, ds.transform)
    with rasterio.open(p) as ds:
        if (
            ds.count != 1
            or ds.descriptions != (expected,)
            or (ds.height, ds.width) != (size, size)
        ):
