...
def _npz(path: Path, key: str) -> np.ndarray:
    with np.load(path, allow_pickle=False) as z:
        return np.asarray(z[key] if key in z else z["arr_0"])
...
def load_records(path: str | Path, task: str | None = None) -> list[Record]:
    ...
    for row in rows:
        ...
        if row_task not in {"af", "bs"} or split not in {"train", "val"}:
            raise ValueError("invalid task or split")
        if record_id in ids:
            raise ValueError(f"duplicate record id {record_id!r}")
        ids.add(record_id)
        resolve = lambda value: (
            Path(value) if Path(value).is_absolute() else (manifest.parent / value)
        ).resolve()
        x_path, y_path = resolve(str(row["x_path"])), resolve(str(row["y_path"]))
        valid_path = resolve(str(row["valid_path"])) if row.get("valid_path") else None
        for data_path in (x_path, y_path, valid_path):
            if data_path is None:
                continue
            if data_path in paths and paths[data_path] != split:
                raise ValueError(f"data path {data_path} occurs in both train and val")
        ...
        result.append(
            Record(
                ...
                y_path,
                valid_path,
                tuple(map(str, row.get("feature_names", []))),
            )
        )
    if not result:
...
def _read_record(record: Record) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if x.ndim != 3 or raw_y.ndim != 2 or not np.issubdtype(raw_y.dtype, np.integer):
        raise ValueError(f"invalid NPZ shapes/types for {record.id}")
    y = raw_y.astype(np.int64, copy=False)
    raw_valid = (
        _npz(record.valid_path, "valid")
        if record.valid_path
        else np.ones_like(y, dtype=bool)
    )
    if raw_valid.dtype != np.bool_ and (
        not np.issubdtype(raw_valid.dtype, np.number)
        or not np.isfinite(raw_valid).all()
        or not np.isin(raw_valid, (0, 1)).all()
    ):
        raise ValueError(f"invalid valid mask for {record.id}")
    valid = raw_valid.astype(bool, copy=False)
    high = 1 if record.task == "af" else 3
    if (
        x.shape[1:] != y.shape
        or valid.shape != y.shape
        or np.any((y != IGNORE) & ((y < 0) | (y > high)))
    ):
        raise ValueError(f"invalid labels/shapes for {record.id}")
    return x, y.astype(np.uint8, copy=False), valid & (y != IGNORE)


def _best_af_threshold(
    reference: list[np.ndarray], probability: list[np.ndarray]
) -> tuple[float, float]:
    truth = np.concatenate([a.reshape(-1) for a in reference]).astype(bool, copy=False)
    score = np.concatenate([a.reshape(-1) for a in probability]).astype(
        np.float32, copy=False
    )
    if not np.isfinite(score).all():
        raise ValueError("AF validation probabilities must be finite")
    ...
    tp = np.cumsum(truth, dtype=np.int64)[positions]
    predicted = positions + 1
    f1 = 2.0 * tp / (predicted + positives)
...
def _lgb():
    try:
        ...
        raise RuntimeError(
            "LightGBM is required; install the pinned competition environment"
        ) from exc
    return lgb
...
def _feature_names(records: list[Record], channels: int) -> list[str]:
    ...
    if not names or len(names) != channels or len(set(names)) != channels:
        raise ValueError(
            "every record requires unique ordered feature_names for all channels"
        )
...
def _flat(x: np.ndarray, valid: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    return x.reshape(x.shape[0], -1)[:, positions].T.astype(
        np.float32, copy=False
    ), positions
...
def _sample_train(
    ...
    xs: list[np.ndarray] = []
    ys: list[np.ndarray] = []
    population = np.zeros(classes, dtype=np.int64)
    selected = np.zeros(classes, dtype=np.int64)
    channels: int | None = None
    for r in records:
        x, y, valid = _read_record(r)
        channels = x.shape[0] if channels is None else channels
        if x.shape[0] != channels:
            raise ValueError("channel count differs within task")
        for c in range(classes):
            ...
            if len(locations):
                part = x.reshape(x.shape[0], -1)[:, locations].T.astype(
                    np.float32, copy=False
                )
                xs.append(part)
                ys.append(np.full(len(locations), c, dtype=np.uint8))
                selected[c] += len(locations)
    ...
    info = {
        ...
        "population_counts": population.astype(int).tolist(),
        "sampled_counts": selected.astype(int).tolist(),
        "per_chip_per_class_cap": per_chip_class,
        "prior_weights": prior_weights.astype(float).tolist(),
    }
    return features, labels, weights, info


def _sample_validation(
    records: list[Record], task: str, max_pixels: int, seed: int
...
def _predict_proba(booster: Any, x: np.ndarray, chunk_pixels: int) -> np.ndarray:
    if x.ndim != 3:
        raise ValueError(f"x must be CHW, got {x.shape}")
    flat = x.reshape(x.shape[0], -1).T
    pieces = []
    for offset in range(0, len(flat), chunk_pixels):
        part = np.array(
            flat[offset : offset + chunk_pixels], dtype=np.float32, copy=True
        )
...
def predict_tree_features(
    ...
    if feature_names is None or list(feature_names) != list(meta["feature_names"]):
        raise ValueError(
            "feature_names must exactly match the model's ordered feature metadata"
        )
    probability = _predict_proba(
        lgb.Booster(model_file=str(root / "model.txt")), x, chunk_pixels
    )
    if meta["task"] == "af":
        score = probability[..., 0]
    ...
    if weights.shape != (4,):
        raise ValueError("invalid class_multipliers in metadata")
...
def _score_bs(
    ...
    burn = float(micro_iou_burn(reference, prediction))
    severity = micro_iou_severity(reference, prediction)
    mean = float(sum(severity.values()) / 3)
    return {
        "IoU_burn": burn,
        "mIoU_sev": mean,
        **{f"IoU_{c}": float(severity[c]) for c in (1, 2, 3)},
    }, 0.35 * burn + 0.30 * mean


def _validation_probabilities(
    booster: Any, records: list[Record]
) -> tuple[list[np.ndarray], list[np.ndarray], list[np.ndarray]]:
    """Run expensive full-chip validation inference once, then reuse it for tuning."""
    reference: list[np.ndarray] = []
    probability: list[np.ndarray] = []
    permitted: list[np.ndarray] = []
    for r in records:
        x, y, valid = _read_record(r)
        ...
        reference.append(np.where(valid, y, 0).astype(np.uint8))
...
def _full_validation(
    ...
    *,
    ...
    if task == "af":
        prediction = [
            np.where(valid, item[..., 0] >= threshold, 0).astype(np.uint8)
            for item, valid in zip(probability, permitted)
        ]
        f1 = float(micro_f1_af(reference, prediction))
        return {"F1_af": f1, "threshold": threshold}, f1
    if multipliers is None:
        raise ValueError("BS requires class multipliers")
    prediction = [
        np.where(valid, (item * multipliers).argmax(axis=-1), 0).astype(np.uint8)
        for item, valid in zip(probability, permitted)
    ]
...
def train_tree_task(
    ...
    *,
    ...
    if any(r.task != task for r in records):
        raise ValueError("train_tree_task takes a single task")
    ...
    channels = x.shape[1]
    names = _feature_names(records, channels)
    lgb = _lgb()
    params: dict[str, Any] = {
        "objective": "binary" if task == "af" else "multiclass",
        "metric": "binary_logloss" if task == "af" else "multi_logloss",
        "learning_rate": 0.06,
        "num_leaves": 63,
        "max_depth": -1,
        "min_data_in_leaf": 80,
    ...
    train_set = lgb.Dataset(
        x, label=y, weight=weight, feature_name=names, free_raw_data=True
    )
    ...
    validation_reference, validation_probability, validation_valid = (
        _validation_probabilities(booster, val_rows)
    )
    if task == "af":
        af_ref = [
            item[valid] for item, valid in zip(validation_reference, validation_valid)
        ]
        af_prob = [
            item[..., 0][valid]
            for item, valid in zip(validation_probability, validation_valid)
        ]
        best_threshold, best = _best_af_threshold(af_ref, af_prob)
        best_metrics, _ = _full_validation(
        ...
        best_metrics, best = _full_validation(
            validation_reference,
            validation_probability,
            validation_valid,
            task,
        ...
        for values in itertools.product((0.7, 1.0, 1.3), repeat=3):
            ...
            if score > best:
                best, best_multiplier, best_metrics = score, candidate, metrics
        best_threshold = None
        multipliers = best_multiplier.astype(float).tolist()
    meta = {
        "format_version": 1,
        "task": task,
        "channels": channels,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Train compact LightGBM pixel baselines from competition NPZ chips"
    )
    parser.add_argument("--records", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--task", choices=("af", "bs", "all"), default="all")
    parser.add_argument("--max-pixels", type=int, default=800_000)
    parser.add_argument("--val-pixels", type=int, default=200_000)
    parser.add_argument("--num-threads", type=int, default=2)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--rounds", type=int, default=1000)
    args = parser.parse_args(argv)
    tasks = ("af", "bs") if args.task == "all" else (args.task,)
    result = []
    for task in tasks:
        result.append(
            train_tree_task(
                load_records(args.records, task),
                Path(args.output) / task,
                max_pixels=args.max_pixels,
                val_pixels=args.val_pixels,
