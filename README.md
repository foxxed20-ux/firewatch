...
**Результат на платформе организаторов: 0,7319, статус `completed`.** Отправленный CSV побайтно совпадает с проверенной поставкой V3. Это результат платформы; компоненты и разделение public/private в интерфейсе не раскрыты. Наша внутренняя validation-оценка — 0,707178. [Подтверждение отправки и результата](docs/evidence/organizer-result-v3.json).

В исходниках также подготовлены Google Maps и поиск/предпросмотр Sentinel-2 через Earth Engine. [Инструкция настройки](docs/GOOGLE_CLOUD_SETUP.md) описывает необходимые Cloud-права и проверки; наличие кода не означает включение этой интеграции на публичном сервере. RGB-превью не заменяет полный набор входных данных модели AF/BS.
...
### Единый комплект для жюри

[Скачать `FireWatch-models-ensemble-v3-licensed.zip`](https://firewatch.151.247.25.189.nip.io/downloads/FireWatch-models-ensemble-v3-licensed.zip) — 17 820 328 байт. Внутри код обучения и инференса, выбранные веса, готовый `submission.csv`, доказательства проверок, паспорт, исследование, презентация, `LICENSE` и `NOTICE`. Сырые спутниковые данные получаются отдельно у организаторов; полный код веб-сервиса находится в этом репозитории. [Состав и использование архива](docs/MODEL_DELIVERY.md).

```text
SHA-256: dc34115c29b8dd6bb119c9fb2367d2f07478a8fb570f11190ad0d554b0f58854
```

Проверены CRC архива из 127 файлов и SHA-256 всех 126 элементов manifest. Все 124 файла исходной поставки сохранены побайтно; добавлены только лицензия, уведомления и обновлённый manifest. Код и веса соответствуют комплекту, из которого прошли 22 модельных теста и четыре реальные сцены на CPU. [Проверка упаковки с лицензией](docs/evidence/licensed-archive-v3.json) · [Проверка публичного скачивания](artifacts/service/public-jury-download-v3-licensed.json).

[Исходный архив v3](https://firewatch.151.247.25.189.nip.io/downloads/FireWatch-models-ensemble-v3.zip) сохранён для воспроизводимости с прежним SHA-256 `45317f541bb60b77d67b5ca4729c939bc34446e08097c1b39b1a0f4ce7e96247`. Его [результаты приёмки](docs/evidence/model-v3-release-summary.json) относятся к неизменённому коду и весам; новая упаковка не является новой моделью или измерением качества.
...
| [`SECURITY.md`](SECURITY.md) | Поддерживаемые версии и приватное сообщение об уязвимости |
| [`CODE_OF_CONDUCT.md`](CODE_OF_CONDUCT.md) | Правила общения, модерация и рассмотрение жалоб |
| [`docs/GOOGLE_CLOUD_SETUP.md`](docs/GOOGLE_CLOUD_SETUP.md) | Настройка Google Maps и поиска Sentinel-2 через Earth Engine |
| [`LICENSE`](LICENSE), [`NOTICE`](NOTICE) | Apache-2.0 для собственного кода и уведомления об авторстве |

## Поддержка и участие

Вопросы по запуску, воспроизводимые ошибки и предложения оставляйте в разделе задач [GitVerse](https://gitverse.ru/hackrus.experts/kosmo-krasnoiarsk_4_th_try_106) или в [GitHub Issues](https://github.com/foxxed20-ux/firewatch/issues). Укажите версию, среду и шаги воспроизведения; исключите из вложений секреты и конкурсные данные. Проект сопровождает команда `4_th_try` без гарантированного SLA.

Для изменений следуйте [правилам участия](CONTRIBUTING.md) и [кодексу поведения](CODE_OF_CONDUCT.md). Уязвимости сообщайте приватно по [политике безопасности](SECURITY.md), контакт команды — Telegram [@F0XXED](https://t.me/F0XXED).

## Лицензия

Собственный исходный код FireWatch распространяется по [Apache License 2.0](LICENSE). Авторство и уведомления сохранены в [NOTICE](NOTICE). Лицензия позволяет использовать, изменять и распространять код при соблюдении её условий; она не предоставляет права на чужие материалы.

Сторонние библиотеки и картографические материалы сохраняют свои лицензии. Конкурсные данные и отдельно скачиваемые веса и результаты не получают лицензию Apache-2.0 автоматически: их происхождение и ограничения описаны в [THIRD_PARTY.md](THIRD_PARTY.md). Исторический архив `FireWatch-models-ensemble-v3.zip` остаётся неизменным; актуальные условия собственного кода опубликованы в этом репозитории.
