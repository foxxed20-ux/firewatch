...
Все показатели качества ниже рассчитаны на одном зафиксированном validation split. Эта же выборка использована для выбора checkpoint, порогов, весов ансамбля и множителей классов; закрытый test Score неизвестен.

| Комплект | F1 AF | IoU гари | mIoU severity | Validation Score |
|---|---:|---:|---:|---:|
| `official-tree-v1` | 0.902135 | 0.549447 | 0.554863 | 0.674512 |
| `official-ensemble-v2` | 0.929509 | 0.576754 | 0.585079 | 0.702716 |
| `official-ensemble-v3` | **0.929509** | **0.586689** | **0.588362** | **0.707178** |

V2 объединяет AF-вероятности нейросети и LightGBM в отношении 25:75. BS использует LightGBM с множителями `[1, 3, 3, 0.8]`. Оба комплекта прошли реальный CPU smoke на четырёх сценах с заблокированными сетевыми подключениями. V1 также прошёл полный конкурсный прогон: 269 чипов, заголовок и 447 строк данных, точное совпадение ключей с шаблоном, RLE round-trip и непересекающиеся BS-классы. Наблюдавшееся время V1 — 466.85 секунды во время параллельного обучения; это не изолированный скоростной benchmark.

Подробности, лицензии и ограничения — [технический отчёт](docs/MODEL_REPORT.md). Точные наблюдавшиеся версии: [Colab training](docs/environment/colab-training.json), [Windows CPU](docs/environment/windows-cpu-inference.json). Для воспроизведения соответствующей среды предусмотрены файлы `docs/environment/requirements-*.txt`.

После выбора рецепта комплект можно собрать так:
...
python -m ops.validate_submission --submission submission.csv --template data/test/sample_submission.csv --report submission_validation.json
```

Итоговая постобработка и метрики выбранного комплекта находятся в `manifest.json`, поля `tasks.*.postprocess` и `tasks.*.selection`. Диагностические метрики внутри отдельных checkpoint/metadata относятся к исходному обучению компонентов; для ансамбля они не заменяют оценку всего комплекта.

Полный автономный прогон V2 обработал 269 test-чипов за 540,30 секунды и сформировал 447 строк данных с заголовком. Независимая проверка подтвердила ключи, порядок, канонический RLE и отсутствие пересечений классов. Повторный CPU-инференс сохранённых весов на всех 129 validation-чипах воспроизвёл три метрики с нулевой дельтой. Это не повтор обучения с нуля; замер времени выполнен во время параллельного обучения.

Сравнение XGBoost для BS завершено: небольшой прирост на validation не оправдал добавление второго backend в выбранный комплект. Итоговый V3 сохраняет AF V2 и объединяет BS CNN/LightGBM в отношении 25:75 с множителями `[1, 2.2, 2.2, 1.3]`. Validation Score вырос до 0,707178; условный bootstrap-интервал прироста включает ноль и не учитывает выбор рецепта. Полный CLI V3 обработал 269 чипов за 390,703 с на GPU T4 при параллельном CPU replay. Повторный CPU-инференс 129 validation-чипов воспроизвёл три метрики с нулевой дельтой. V3 прошёл сервисную и браузерную приёмку; V2 остаётся проверенным резервным комплектом. [Протокол и доказательства](docs/evaluation.md).
